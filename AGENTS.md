...
- **`agent-bot-scenarios/src/flows.json` и `public/assets/` намеренно НЕ версионируются git'ом** (оба ранее были случайно закоммичены, сняты с трекинга и добавлены в `.gitignore` коммитом от 2026-08-20). `flows.json` — живые данные редактора `/admin`, правится либо через `/agent-bot/admin/` (UI или `GET`/`POST /agent-bot/admin/api/flows`, Basic Auth password = личный access token админа Chatwoot), либо прямой правкой на дропле — но **перед любым `git checkout`/`git reset --hard` на дропле сначала бэкапить живой `flows.json` за пределы репозитория**, иначе потеряется при переключении веток. `public/assets/` — билд-артефакты, пересоздаются `assets:precompile`, руками не трогать.
