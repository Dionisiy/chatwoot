...
- **На дропле одна ветка — `custom/slideedu`, она же единственный источник правды.** (Обновление 2026-08-20: раньше на дропле реально стояла отдельная `custom/slideedu-stage` со 128+ незапушенными коммитами — расследовали и слили: все коммиты дропла оказались уже задублированы в историю `custom/slideedu` под другими хэшами, ничего не потеряли, `-stage` удалена и локально на дропле, и на GitHub. Если снова заведётся ветка типа `-stage`/`-prod` — сразу сверять `git log branchA..branchB` в обе стороны, не доверять на слово названию.)
- **SSH-ключ дропла к GitHub почи́нен (2026-08-20).** У пользователя `chatwoot` отдельный ed25519-ключ `~/.ssh/github_deploy_key`, прописан в `~/.ssh/config` под `Host github.com`, добавлен в GitHub-репозиторий как Deploy Key с правом записи (Settings → Deploy keys). `git fetch`/`pull`/`push` от `chatwoot` (`sudo -u chatwoot -i ...`) работают напрямую. Снова словили `Permission denied (publickey)` — проверьте, что команда идёт от юзера `chatwoot`, а не root/другого.
...
- **`agent-bot-scenarios/src/flows.json` и `public/assets/` намеренно НЕ версионируются git'ом** (оба ранее были случайно закоммичены, сняты с трекинга и добавлены в `.gitignore` коммитом от 2026-08-20). `flows.json` — живые данные редактора `/admin`, правится либо через `/agent-bot/admin/` (UI или `GET`/`POST /agent-bot/admin/api/flows`, Basic Auth password = личный access token админа Chatwoot), либо прямой правкой на дропле — но **перед любым `git checkout`/`git reset --hard` на дропле сначала бэкапить живой `flows.json` за пределы репозитория**, иначе потеряется при переключении веток. **После прямой правки файла на диске бот НЕ видит изменений** (flowStore кэширует flows в памяти; «подхват без рестарта» работает только для сохранений через `/admin`) — сбросить кэш: `curl -u admin:<админ-токен> http://localhost:8010/admin/api/flows` (GET вызывает reloadFlows) либо перезапустить pm2-процесс. `public/assets/` — билд-артефакты, пересоздаются `assets:precompile`, руками не трогать.
- **Ruby на дропле — через RVM, не rbenv** (в отличие от локальной разработки). Systemd-юниты подставляют `PATH`/`GEM_HOME` на `/home/chatwoot/.rvm/...` явно; при ручных SSH-командах `bundle`/`rails` доступны только в login-shell (`ssh host "bash -lc '...'"` или `sudo -u chatwoot -i ...`, подхватывает `/etc/profile.d/rvm.sh`) — обычный `ssh host "bundle ..."` без логин-шелла даст `bundle: command not found`.
